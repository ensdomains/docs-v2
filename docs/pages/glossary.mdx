...
## First Layer Names {{ id: "first-layer" }}

First layer names are names one "layer" deep into the resolution process. Examples of first-layer names include `luc.eth`, `vitalik.eth`, `luc.computer`, etc.

### Not First Layer
Names that are **NOT** first layer are `hello.luc.eth`, `foo.bar.hello.xyz`, etc.

## Legacy Vocabulary {{ title: "Legacy", id: "legacy" }}

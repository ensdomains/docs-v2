...
Second-level domain. This refers to a subname/subdomain of a top-level domain. For example, `name.eth` and `name.com` are both second-level names. A subname of a 2LD is a third-level domain or 3LD.

## CCIP-read

Another name for [EIP-3668](https://eips.ethereum.org/EIPS/eip-3668). This allows data for an Ethereum call or transaction to be fetched offchain and then securely verified on-chain. With respect to ENS, this is typically used for offchain subnames that don't exist in the core Registry.
...
Another name for a [Subname](#subname).

## Subgraph

An indexed collection of data using TheGraph protocol. In this documnetation portal, "the subgraph" usually refers to the official ENS subgraph maintained by ENS Labs. This is a useful offchain service that allows clients to query for information about names or accounts.

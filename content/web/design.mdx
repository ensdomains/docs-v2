...
- **Not all ENS names end with .eth**: ENS supports .eth and most DNS TLDs such as .com, .xyz, and 1200+ others. To remain future-proof, a correct implementation of ENS treats any dot-separated name as a potential ENS name and will attempt a look-up.
...
- **Always provide the user a way to display the full Ethereum address**: Notice how if you type "ens.eth" in the [example above](#send_transaction), the resolved ETH address appears under the name. This is especially important in high-risk situations, such as when the user is about to send a transaction or interact with a smart contract.
...
You can offer free ENS names to your users which would not only improve their experience in your application, but also across the Ethereum ecosystem. See [how to issue subdomains](/web/subdomains).
...
[**Thorin**](https://thorin.ens.domains/) is a react component library for the ENS design system. It provides a set of components that make it easier to follow the guidelines and best practices described above.

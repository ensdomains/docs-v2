...
export default function normalizePages({
    ...
    if (locale) {
        for (const a of list)
            a.locale === locale ? hasLocale.set(a.name, true) : undefined;
    }
    ...
    for (const a of list
        ...
        if (
            ...
            switch (activeType) {
                case 'nav':
                    activeIndex =
                        flatPageDirectories.length +
                        normalizedChildren.activeIndex;
                    break;
                case 'docs':
                    activeIndex =
                        flatDocsDirectories.length +
                        normalizedChildren.activeIndex;
                    break;
            }
        }
            ...
            switch (type) {
                case 'nav':
                    pageItem.children.push(
                        ...normalizedChildren.pageDirectories
                    );
                    ...
                    if (
                        normalizedChildren.flatPageDirectories.length === 0 &&
                        normalizedChildren.flatDirectories.length > 0
                    ) {
                        pageItem.firstChildRoute =
                            normalizedChildren.flatDirectories.at(0).route;
                ...
                default:
                    if (isCurrentDocsTree) {
                        docsItem.children.push(
                            ...normalizedChildren.docsDirectories
                        );

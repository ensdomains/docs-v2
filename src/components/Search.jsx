                                ...
                                queries: [
                                    {
                                        ...
                                        params: {
                                            hitsPerPage: 5,
                                            highlightPreTag:
                                                '<mark class="underline bg-transparent text-ens-500">',
...
function NoResultsIcon(properties) {
    return (
        <svg viewBox="0 0 20 20" fill="none" aria-hidden="true" {...properties}>
            <path
                strokeLinecap="round"
...
function LoadingIcon(properties) {
    const id = useId();
    ...
    return (
        ...
        >
            <div
                id={`${id}-title`}
                aria-hidden="true"
                className="text-sm font-medium text-zinc-900 group-aria-selected:text-ens-500 dark:text-white"
...
const SearchInput = forwardRef(
    ({ autocomplete, autocompleteState, onClose }, inputReference) => {
        const inputProperties = autocomplete.getInputProps({});
                ...
                <input
                    ...
                    onKeyDown={(event) => {
                        if (
                            ...
                            inputProperties.onKeyDown(event);
                        }
                    }}
                />
                {autocompleteState.status === 'stalled' && (
                    <div className="absolute inset-y-0 right-3 flex items-center">
                        <LoadingIcon className="h-5 w-5 animate-spin stroke-zinc-200 text-zinc-900 dark:stroke-zinc-800 dark:text-ens-400" />
                    </div>
                )}
            </div>
        );
...
function AlgoliaLogo(properties) {
    return (
        <svg
            viewBox="0 0 71 16"
            role="img"
...
function SearchButton(properties) {
                            ...
                            <div {...autocomplete.getRootProps({})}>
                                <form
                                    ...
                                    {...autocomplete.getFormProps({
                                        inputElement: inputReference.current,
                                    })}
                                >
                                    <SearchInput
                                        ref={inputReference}
